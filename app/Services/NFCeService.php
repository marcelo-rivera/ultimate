...
class NFCeService{
	...
	public function gerarNFCe($venda){
		...
		$lines = $venda->payment_lines;
		$tipo = '01';
		if(count($lines) > 1) $tipo = '99';
		else{
			if($lines[0]->method == 'card') $tipo = '03';
		}
		$stdDetPag->tPag = $tipo; 
		$stdDetPag->vPag = $this->format($venda->valor_recebido); //Obs: deve ser informado o valor pago pelo cliente
